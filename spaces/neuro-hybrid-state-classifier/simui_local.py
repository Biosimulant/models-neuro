#!/usr/bin/env python3
"""Run neuro-hybrid-state-classifier with SimUI."""
...
def main() -> None:
    ...
    ui = Interface(
        world,
        title=space.get("title", "Neuro: Hybrid State Classifier"),
        description=space.get("description", "Hybrid mechanistic + ONNX classifier demo."),
        controls=[
            Number("duration", float(duration), label="Duration", minimum=0.001, maximum=1000.0, step=0.001),
            Number("tick_dt", float(tick_dt), label="tick_dt", minimum=0.0001, maximum=10.0, step=0.0001),
            Button("Run"),
        ],
